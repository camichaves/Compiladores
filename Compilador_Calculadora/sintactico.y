...
Calc	 :  Exp PTOCOMA {printf ("Linea %d = %4.2f\n",linea,$1);} | Exp error PTOCOMA | error Exp PTOCOMA |error "\n" {yyerror("Falta punto y coma");};
...
/* con el simbolo de $$ asignamos el valor semantico de toda
  la accion de la derecha y se la asignamos al no terminal de
   ...
   la posicion en la que se encuentra es decir si tenemoscaca
...
Exp :   NUMERO {$$=$1;}
			...
			|Exp DIV Exp {$$=$1/$3;} 
			|PAA Exp PAC {$$=$2;}
      |SIN PAA Exp PAC {$$=sin($3);}
      |COS PAA Exp PAC {$$=cos($3);}
      |POW PAA Exp COMMA Exp PAC {$$=pow($3,$5);}
      |POW PAA error PAC {yyerror("Error en POW");}
      |PAA error PAC {yyerror("Falta parentesis");};
